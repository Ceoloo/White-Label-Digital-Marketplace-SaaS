...
## AI-fulfilled services (sellable, then generated by Claude)

Admins can sell **services fulfilled by AI**, not just digital downloads. A
product with `type: "service"` carries a `service` config — the AI instructions
plus the input fields the customer fills in. It flows through the same catalog,
cart, and checkout as any product.

Lifecycle:

1. **Admin** creates the service at `/admin/products` (Product type →
   *AI-fulfilled service*): AI instructions, customer input fields (one label
   per line), optional delivery format and system prompt.
2. **Customer** buys it through normal checkout. Each purchased service unit
   spawns a `ServiceRequest` (status `pending`) tied to the buyer.
3. **Customer** opens **`/services`** ("My AI services"), fills in the inputs,
   and clicks *Generate* → `POST /api/services/[id]/fulfill` runs the AI and
   stores the deliverable (`processing` → `completed`), which they can view and
   download.

**Admin oversight**: `/admin/services` lists every service request across all
customers — status counters plus a table with the customer, order, fulfillment
status, and a viewer for the customer inputs and generated deliverable.

**AI layer** ([`lib/ai.ts`](lib/ai.ts)) — pluggable provider so the app can run
on a **private local model**, Anthropic, or a no-model simulation:

- **`local` (private)** — any OpenAI-compatible endpoint you run yourself
  (Ollama, LM Studio, vLLM, LocalAI, …). Set `LOCAL_AI_BASE_URL` (the
  OpenAI-compatible base, incl. `/v1`) and `LOCAL_AI_MODEL` (+ optional
  `LOCAL_AI_API_KEY`). Fulfillment is a standard Chat Completions call — **no
  data leaves your infrastructure**, which complements the Privacy Suite.
- **`anthropic`** — Claude (default model `claude-opus-4-8`) via the official
  `@anthropic-ai/sdk` when `ANTHROPIC_API_KEY` is set.
- **`none`** — deterministic **simulated** deliverable, so the flow runs and
  demos with zero setup.

`AI_PROVIDER` forces a choice; otherwise selection is automatic — a configured
private local model is preferred, then Anthropic, then the simulation. All
endpoints/keys are read from the environment and never hardcoded, and any
live-call failure degrades to the simulation rather than erroring. The active
provider is surfaced to customers (`/services`) and admins (`/admin/services`).
The seeded demo service ("AI Landing Page Copywriter") lets you try the whole
flow immediately.
...
- **AI services** — `AI_PROVIDER` (`local` / `anthropic` / `none`, or blank for
  auto); private local model via `LOCAL_AI_BASE_URL` / `LOCAL_AI_MODEL` /
  `LOCAL_AI_API_KEY`; hosted via `ANTHROPIC_API_KEY`. Blank = simulated.
