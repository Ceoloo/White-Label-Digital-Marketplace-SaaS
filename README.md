...
**AI layer** ([`lib/ai.ts`](lib/ai.ts)): when `ANTHROPIC_API_KEY` is set, the
deliverable is generated by Claude (default model `claude-opus-4-8`) via the
official `@anthropic-ai/sdk`. With no key, a deterministic **simulated**
deliverable is returned so the flow runs and demos with zero setup. The key is
read from the environment and never hardcoded; any live-call failure degrades to
the simulation rather than erroring. The seeded demo service ("AI Landing Page
Copywriter") lets you try the whole flow immediately.
...
- **AI services** — `ANTHROPIC_API_KEY` (fulfill services with Claude; blank =
